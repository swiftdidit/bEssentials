...
public class Tpahere extends Command implements TabExecutor {
    private final bEssentials core;
    public Tpahere(bEssentials core) {
        super("tpahere", "bessentials.admin");
        this.core = core;
    }
    @Override
    public void execute(CommandSender sender, String[] args) {
        if (!(sender instanceof ProxiedPlayer)) {
            ...
            if (sender.hasPermission("bessentials.admin") || sender.hasPermission("bessentials.tpahere")) {
                switch (args.length) {
                    case 1:
                        ...
                        core.getFiles().pubJedis("bungee", "tpahere:sent");
                        core.getFiles().getJedis().set("tpaHereTargetName", target.getName());

                        scheduleExpirationTask(sender, target);
                        break;

                    default:
                        Message.of("messages.player.tpa.tpahere-usage").send(sender);
                }
            } else {
    ...
    private void scheduleExpirationTask(CommandSender sender, ProxiedPlayer target) {
        TpahereExpirationTask expirationTask = new TpahereExpirationTask(core, sender, target);
...
class TpahereExpirationTask {
    ...
    public TpahereExpirationTask(bEssentials core, CommandSender sender, ProxiedPlayer target) {
        this.core = core;
        this.sender = sender;
        this.target = target;
        this.expirationTime = System.currentTimeMillis() + core.getFiles().getConfig().get().getInt("settings.tpahere-expiretime") * 1000L;
        this.executed = false;
    }
